class HistoricSale < ApplicationRecord
  belongs_to :store
  belongs_to :department
  validates_presence_of :store_id, :department_id, :historic_date
  ...
  class << self
    ...
    def import_row(row)
      ...
      historic_sale = HistoricSale.find_or_initialize_by(
        store_id: store.id,
        department_id: department.id,
        historic_date: row[2]
      )

      historic_sale.update_attributes(
