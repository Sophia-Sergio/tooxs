class SaleReal < ApplicationRecord
  ...
  scope :with_store, lambda { |value| where('store_id = (?)', value) if value }
  scope :with_department, lambda { |value| where('department_id = (?)', value) if value }
  scope :with_year, lambda { |value| where('year = (?)', value) if value }
