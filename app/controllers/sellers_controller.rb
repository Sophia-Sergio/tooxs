  ...
  def sale_plan(seller,year,month)
    ...
    def sale_real_per_week(seller,year,month)
      ...
      (1..dayNow[:week]).each do |w|
        ...
        day = Array.new(7)
        (1..7).each do |d|
          day[dayCount] = SellerSale.where(month: month, seller: seller.id, week: w, day: d, department: @dep, year: @year).sum("sale")
          dayCount +=1
        end
    ...
    def week_sale_plan(w)
      ...
      sale_plan_day = {}
      w.each do |d|
        sale_plan_day[d.sale_date.strftime('%Y%m%d').to_sym] = d.to_hour
      end
