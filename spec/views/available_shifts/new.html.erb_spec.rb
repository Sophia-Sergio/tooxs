...
RSpec.describe "available_shifts/new", type: :view do
  before(:each) do
    assign(:available_shift, AvailableShift.new(
      :store => nil,
      :num => 1,
      :name => "MyString",
      :month => 1,
  ...
  it "renders new available_shift form" do
    render

    assert_select "form[action=?][method=?]", available_shifts_path, "post" do
